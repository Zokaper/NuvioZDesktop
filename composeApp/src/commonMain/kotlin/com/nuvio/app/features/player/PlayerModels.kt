 ...
 * needs the engine's own answer: mpv will not leave `paused-for-cache` until its own cache says so,
 * and ExoPlayer will not leave `STATE_BUFFERING` until its rebuffer condition is satisfied - five
 * seconds on mobile, not the one second a buffered-ahead heuristic used to accept.
 */
