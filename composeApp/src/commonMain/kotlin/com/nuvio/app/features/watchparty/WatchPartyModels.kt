 ...
 * pitch shift, so the guest seeks and accepts the stall.
 */
 ...
 * completes: the shared clock runs on through the reload, so the guest resumes behind, measures the
 * same drift again, and seeks again. Leading by roughly the cost of resuming is what makes one
 ...
 *
 * A seek is the expensive option, not the safe one: it reloads, the shared clock runs on through
 * the reload, and the guest is handed a *larger* drift on the next pass - a loop that settles at
 * roughly the reload cost and never leaves. So the nudge band has to be wide enough, and the nudge
 * itself strong enough, to be the ordinary answer; seeking is reserved for a gap no nudge can close.
 *
